#!/bin/bash
# deploy-livechat.sh — light-livechat SFU 서버 배포/운영 스크립트
# author: kodeholic (powered by Claude)
#
# 사용법:
#   ./deploy-livechat.sh patch     — 전체 배포 (stop → 백업 → git pull → build → start)
#   ./deploy-livechat.sh start     — 서버 시작
#   ./deploy-livechat.sh stop      — 서버 종료
#   ./deploy-livechat.sh restart   — 서버 재시작
#   ./deploy-livechat.sh status    — 프로세스 상태 확인
#   ./deploy-livechat.sh setup     — 최초 1회: Rust 설치 + 빌드 의존성 + 초기 배포
#   ./deploy-livechat.sh log       — 최근 로그 tail

set -euo pipefail

# --- 설정 변수 ---
APP_NAME="light-livechat"
BASE_DIR="$HOME/livechat"
SRC_DIR="${BASE_DIR}/src"
BIN_DIR="${BASE_DIR}/bin"
BACKUP_DIR="${BASE_DIR}/backup"
LOG_DIR="${BASE_DIR}/logs"
PID_FILE="${BASE_DIR}/livechat.pid"
ENV_FILE="${BASE_DIR}/.env"

GIT_REPO_URL="https://github.com/kodeholic9/light-livechat.git"
GIT_BRANCH="main"

# 바이너리명 (Cargo.toml [[bin]] name = "livechatd")
BIN_NAME="livechatd"
...
do_clone_and_build() {
    ...
    if ! command -v cargo &>/dev/null; then
        ...
        if ! command -v cargo &>/dev/null; then
            err "cargo를 찾을 수 없습니다. 먼저 ./deploy-livechat.sh setup 실행하세요."
            return 1
        fi
    fi
...
do_backup() {
    ...
    if [ -f "${BIN_DIR}/${BIN_NAME}" ]; then
        ...
        local BACKUP_FILE="${BACKUP_DIR}/livechat_${TIMESTAMP}.tar.gz"
        tar -czf "$BACKUP_FILE" -C "$BIN_DIR" .
        ok "백업 완료: ${BACKUP_FILE}"

        # 오래된 백업 정리 (최근 5개만 유지)
        local BACKUP_COUNT
        BACKUP_COUNT=$(ls -1 "${BACKUP_DIR}"/livechat_*.tar.gz 2>/dev/null | wc -l)
        if [ "$BACKUP_COUNT" -gt 5 ]; then
            ls -1t "${BACKUP_DIR}"/livechat_*.tar.gz | tail -n +6 | xargs rm -f
...
do_patch() {
    ...
    if ! do_clone_and_build; then
        ...
        # 가장 최근 백업에서 복구
        local LATEST_BACKUP
        LATEST_BACKUP=$(ls -1t "${BACKUP_DIR}"/livechat_*.tar.gz 2>/dev/null | head -1)
