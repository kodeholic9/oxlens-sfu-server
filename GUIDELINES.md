...
| T-6   | 진단 공백 해소 (인코더 심층 진단 + 구간별 손실 + 이벤트 타임라인)    | 0.5.4  | ✅   |
...
아래 문서 파일은 **절대 직접 write_file/덮어쓰기하지 않는다**:

- `CHANGELOG.md`
- `TODO.md`
- `GUIDELINES.md`

이유: 전체 덮어쓰기 시 이전 버전 이력이 잘림. git 복원이 가능하나 불필요한 사고.

변경이 필요할 때:

1. 변경 내용을 **채팅창에 코드블록으로** 작성하여 부장이 복붙하도록 유도
2. 삽입 위치를 명확히 지정 (예: "`## [0.5.3]` 바로 위에 삽입")
3. 기존 내용 삭제/수정이 필요하면 diff 형태로 제시

예외: `TELEMETRY.md` 등 전체 구조가 바뀌는 경우는 부장 확인 후 전체 쓰기 허용.
