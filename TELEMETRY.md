...
| `jitterBufferDelay` | JB 누적 지연 (÷ emitted = 평균) | — | — | — |
| `jitterBufferEmittedCount` | JB에서 나온 샘플/프레임 수 | — | — | — |
| **JB 평균 지연** | `delay ÷ emitted × 1000`ms | < 50ms | 50-100ms | > 100ms |
...
**JB 평균 지연 계산:**
```
jb_delay_ms = (jitterBufferDelay / jitterBufferEmittedCount) × 1000
```
- 이 값은 체감 지연의 핵심 요소. RTT/2 + SFU 처리시간 + JB 지연 = 총 지연
...
| `twcc_feedback` | TWCC feedback 전송 여부 | 구현됨 | 미구현 (현재 WARN) |
...
| S-2 (코덱) | ✅ | ✅ | 3초 주기 |
| A (Publish) | ✅ | ✅ | delta bitrate 포함 |
| B (SFU) | ✅ | ✅ | 타이밍 + RTCP 카운터 |
| C (Subscribe) | ✅ | ✅ | sourceUser + delta bitrate 포함 |
| Contract | ✅ | ✅ | 8항목 (TWCC는 WARN) |
| 스냅샷 | ✅ | ✅ | 클립보드 복사 |
| 시계열 차트 | 데이터 수집만 | ❌ | 향후 추가 예정 |

### 미구현 (향후)
- TWCC feedback 생성 (서버)
- subscribe RTCP 경로 디버그 카운터 (서버)
- 시계열 차트 (어드민)
